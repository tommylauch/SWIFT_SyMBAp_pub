!**********************************************************************
!           SWIFT_MVS_P.F
!**********************************************************************
!
!                 NO CLOSE ENCOUNTERS
!                 To run, need 3 input files. The code prompts for
!                 the file names, but examples are :
!
!                   parameter file like       param.in
!            planet file like          pl.in
!                   test particle file like   tp.in
!
! Authors:  Hal Levison \& Martin Duncan
! Date:    5/7/93
! Last revision: 12/27/96

      program swift_mvs_p
      include 'swift.inc'

      real*8 xht(3,NTPMAX)
      real*8 vxht(3,NTPMAX)

      real*8 mass(NPLMAX),j2rp2,j4rp4
      real*8 xh(3,NPLMAX)
      real*8 vxh(3,NPLMAX)

      integer istat(NTPMAX,NSTAT),i1st
      integer nbod,ntp,nleft
      integer iflgchk,iub,iuj,iud,iue
      real*8 rstat(NTPMAX,NSTATR)

      real*8 t0,tstop,dt,dtout,dtdump
      real*8 t,tout,tdump,tfrac,eoff

      real*8 rmin,rmax,rmaxu,qmin,rplsq(NPLMAX)
      logical*2 lclose 

      character*80 outfile,inparfile,inplfile,intpfile,fopenstat


!-----
!...    Executable code 

!...    print version number
      call util_version

! Get data for the run and the test particles
      write(*,*) 'Enter name of parameter data file : '
      read(*,999) inparfile
      call io_init_param(inparfile,t0,tstop,dt,dtout,dtdump,
     &         iflgchk,rmin,rmax,rmaxu,qmin,lclose,outfile,fopenstat)

! Prompt and read name of planet data file
      write(*,*) ' '
      write(*,*) 'Enter name of planet data file : '
      read(*,999) inplfile
999   format(a)
      call io_init_pl_array(inplfile,lclose,iflgchk,nbod,mass,xh,
     &         vxh,rplsq,j2rp2,j4rp4)

! Get data for the run and the test particles
      write(*,*) 'Enter name of test particle data file : '
      read(*,999) intpfile
      call io_init_tp_array(intpfile,ntp,xht,vxht,istat,rstat)

c!Initialize initial time and times for first output and first dump
      t = t0
      tout = t0 + dtout
      tdump = t0 + dtdump

      iub = 20
      iuj = 30
      iud = 40
      iue = 60

!...    Do the initial io write
      if(btest(iflgchk,0))  then ! bit 0 is set
         call io_write_frame_array(t0,nbod,ntp,mass,xh,vxh,
     &         xht,vxht,istat,outfile,iub,fopenstat)
      endif
      if(btest(iflgchk,1))  then ! bit 1 is set
         call io_write_frame_r_array(t0,nbod,ntp,mass,xh,vxh,
     &         xht,vxht,istat,outfile,iub,fopenstat)
      endif
      if(btest(iflgchk,2))  then    ! bit 2 is set
         eoff = 0.0d0
         call anal_energy_write_array(t0,nbod,mass,j2rp2,j4rp4,xh,vxh,
     &          iue,fopenstat,eoff)
      endif
      if(btest(iflgchk,3))  then    ! bit 3 is set
         call anal_jacobi_write_array(t0,nbod,ntp,mass,xh,vxh,xht,
     &                     vxht,istat,2,iuj,fopenstat)
      endif

!...    must initize discard io routine
      if(btest(iflgchk,4))  then ! bit 4 is set
         call io_discard_write_array(0,t,nbod,ntp,xh,vxh,xht,vxht,
     &     istat,rstat,iud,'discard.out',fopenstat,nleft)
      endif

      nleft = ntp
      i1st = 0
!***************here's the big loop *************************************
      write(*,*) ' ************** MAIN LOOP ****************** '

      do while ( (t .le. tstop) .and. ((ntp.eq.0).or.(nleft.gt.0)) )
         call step_kdk_p(i1st,t,nbod,ntp,mass,j2rp2,j4rp4,
     &         xh,vxh,xht,vxht,istat,rstat,dt)
         t = t + dt
         if(btest(iflgchk,4))  then    ! bit 4 is set
            call discard_array(t,dt,nbod,ntp,mass,xh,vxh,xht,vxht,
     &     rmin,rmax,rmaxu,qmin,lclose,rplsq,istat,rstat)
            call io_discard_write_array(1,t,nbod,ntp,xh,vxh,xht,vxht,
     &      istat,rstat,iud,'discard.out',fopenstat,nleft)
         else
            nleft = ntp
         endif

! if it is time, output orb. elements, 
         if(t .ge. tout) then
            if(btest(iflgchk,0))  then    ! bit 0 is set
               call io_write_frame_array(t,nbod,ntp,mass,xh,vxh,
     &               xht,vxht,istat,outfile,iub,fopenstat)
            endif
            if(btest(iflgchk,1))  then    ! bit 1 is set
               call io_write_frame_r_array(t,nbod,ntp,mass,xh,vxh,
     &               xht,vxht,istat,outfile,iub,fopenstat)
            endif
            tout = tout + dtout
         endif

! If it is time, do a dump
         if(t.ge.tdump) then
            tfrac = (t-t0)/(tstop-t0)
            write(*,998) t,tfrac,nleft
 998        format(' Time = ',1p1e12.5,': fraction done = ',0pf5.3,
     &            ': Number of active tp =',i4)
            call io_dump_pl_array('dump_pl.dat',nbod,mass,xh,
     &                 vxh,lclose,iflgchk,rplsq,j2rp2,j4rp4)
            call io_dump_tp_array('dump_tp.dat',ntp,xht,vxht,istat,
     &                            rstat)
            call io_dump_param('dump_param.dat',t,tstop,dt,dtout,
     &           dtdump,iflgchk,rmin,rmax,rmaxu,qmin,lclose,outfile)
            tdump = tdump + dtdump

            if(btest(iflgchk,2))  then    ! bit 2 is set
               call anal_energy_write_array(t,nbod,mass,j2rp2,j4rp4,
     &               xh,vxh,iue,fopenstat,eoff)
            endif
            if(btest(iflgchk,3))  then    ! bit 3 is set
               call anal_jacobi_write_array(t,nbod,ntp,mass,xh,vxh,xht,
     &                           vxht,istat,2,iuj,fopenstat)
            endif
         endif

      enddo
!********** end of the big loop from time 't0' to time 'tstop'

! Do a final dump for possible resumption later 

      call io_dump_pl_array('dump_pl.dat',nbod,mass,xh,
     &            vxh,lclose,iflgchk,rplsq,j2rp2,j4rp4)
      call io_dump_tp_array('dump_tp.dat',ntp,xht,vxht,istat,rstat)
      call io_dump_param('dump_param.dat',t,tstop,dt,dtout,
     &         dtdump,iflgchk,rmin,rmax,rmaxu,qmin,lclose,outfile)

      call util_exit(0)
      end    ! swift_mvs.f
!---------------------------------------------------------------------